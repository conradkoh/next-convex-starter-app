...
Upstream template flows live in `apps/webapp/tests/e2e/specs/upstream/` and are tagged `@upstream`. They cover core auth/navigation flows (home, login, anonymous auth, app dashboard, unauthorized access, profile) and admin flows (`/app/system-admin`, `/app/system-admin/users`, `/app/system-admin/google-auth`) and form the regression baseline for the upcoming base UI migration.
...
- Admin flows (`/app/system-admin`, `/app/system-admin/users`, `/app/system-admin/google-auth`) are now covered.
