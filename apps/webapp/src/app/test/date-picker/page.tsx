...
function Field({ label, id, type = 'date', value, onChange, wrapInput = false }: { label: string; id: string; type?: string; value: string; onChange: (value: string) => void; wrapInput?: boolean }) {
  const input = <Input id={id} type={type} value={value} onChange={(e) => onChange(e.target.value)} />;
  return <div className="space-y-2"><Label htmlFor={id}>{label}</Label>{wrapInput ? <div className="ios-date-input-flex">{input}</div> : input}</div>;
}
...
export default function DatePickerTestPage() {
  ...
  return <div className="container mx-auto max-w-4xl space-y-8 px-4 py-8">
    ...
    <Section title="C. Constrained parent (max-w-md)" description="Compare the Safari overflow bug without the wrapper and the flex-wrapper fix."><div className="w-full space-y-4 rounded-md border bg-muted/20 p-4 sm:max-w-md"><Field label="Without ios-date-input-flex" id="constrained-raw" value={value('constrained-raw')} onChange={set('constrained-raw')} /><Field label="With ios-date-input-flex" id="constrained-fixed" value={value('constrained-fixed')} onChange={set('constrained-fixed')} wrapInput /></div></Section>
    <Section title="D. Flex side by side" description="Compare two-field rows with and without the iOS wrapper."><div className="space-y-4"><div className="flex flex-col gap-4 rounded-md border bg-muted/20 p-4 sm:flex-row"><div className="flex-1"><Field label="Start date (raw)" id="flex-raw-start" value={value('flex-raw-start')} onChange={set('flex-raw-start')} /></div><div className="flex-1"><Field label="End date (raw)" id="flex-raw-end" value={value('flex-raw-end')} onChange={set('flex-raw-end')} /></div></div><div className="flex flex-col gap-4 rounded-md border bg-muted/20 p-4 sm:flex-row"><div className="flex-1"><Field label="Start date (fixed)" id="flex-fixed-start" value={value('flex-fixed-start')} onChange={set('flex-fixed-start')} wrapInput /></div><div className="flex-1"><Field label="End date (fixed)" id="flex-fixed-end" value={value('flex-fixed-end')} onChange={set('flex-fixed-end')} wrapInput /></div></div></div></Section>
    <Section title="E. Grid layout (2 columns)" description="Check temporal inputs inside a responsive two-column form grid."><div className="grid grid-cols-1 gap-4 rounded-md border bg-muted/20 p-4 sm:grid-cols-2"><Field label="Grid date" id="grid-date" value={value('grid-date')} onChange={set('grid-date')} wrapInput /><Field label="Grid time" id="grid-time" type="time" value={value('grid-time')} onChange={set('grid-time')} wrapInput /></div></Section>
    <Section title="F. All native temporal types" description="Test the native date, datetime, time, month, and week controls in dark and light modes."><div className="space-y-4">{(['date', 'datetime-local', 'time', 'month', 'week'] as const).map((type) => <Field key={type} label={type} id={`all-${type}`} type={type} value={value(`all-${type}`)} onChange={set(`all-${type}`)} wrapInput />)}</div></Section>
