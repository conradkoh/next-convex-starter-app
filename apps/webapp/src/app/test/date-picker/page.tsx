...
export default function DatePickerTestPage() {
  ...
  return <div className="container mx-auto max-w-4xl space-y-8 px-4 py-8">
    ...
    <Section title="G. Popover + Calendar picker" description="Custom picker comparison: inspect trigger width, popover placement, and calendar contrast."><div className="space-y-4"><div className="w-full rounded-md border bg-muted/20 p-4"><Label>Full-width calendar</Label><Popover><PopoverTrigger asChild><Button variant="outline" className="mt-2 w-full justify-start text-left font-normal"><CalendarIcon className="mr-2 size-4" />{popoverDate ? popoverDate.toLocaleDateString() : 'Pick a date'}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={popoverDate} onSelect={setPopoverDate} /></PopoverContent></Popover></div><div className="w-full rounded-md border bg-muted/20 p-4 sm:max-w-md"><Label>Constrained calendar</Label><Popover><PopoverTrigger asChild><Button variant="outline" className={cn('mt-2 w-full justify-start text-left font-normal', !popoverDate && 'text-muted-foreground')}><CalendarIcon className="mr-2 size-4" />{popoverDate ? popoverDate.toLocaleDateString() : 'Pick a date'}</Button></PopoverTrigger><PopoverContent className="w-auto p-0"><Calendar mode="single" selected={popoverDate} onSelect={setPopoverDate} /></PopoverContent></Popover></div></div></Section>
  </div>;
}
