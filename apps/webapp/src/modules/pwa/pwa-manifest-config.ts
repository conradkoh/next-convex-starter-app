...
/** Shared PWA manifest fields used by the server route and client-side updater. */
export function buildPwaManifest(startUrl: string): MetadataRoute.Manifest {
  return {
...
/** Fallback when JS has not yet set a route-specific manifest link. */
export const DEFAULT_PWA_START_URL = '/app';

/** Build same-origin start_url from the active client route. */
export function getPwaStartUrlFromLocation(pathname: string, search: string): string {
