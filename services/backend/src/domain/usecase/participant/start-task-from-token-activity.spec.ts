...
describe('startTaskFromTokenActivity — enhancer daemon pending path', () => {
  test('starts enhancer daemon pending task on token activity when running job exists', async () => {
    const { sessionId } = await createTestSession('stta-enhancer-daemon');
    const chatroomId = await createChatroom(sessionId, ['enhancer']);
    await joinParticipant(sessionId, chatroomId, 'enhancer');

    const taskId = await seedPendingTask(chatroomId, {
      assignedTo: 'enhancer',
      queuePosition: 0,
      content: 'enhancer daemon task',
    });

    // Get a real user ID — no index available in fake convex-test, use first user
    const userId = await t.run(async (ctx) => {
      const users = await ctx.db.query('users').collect();
      return users[0]!._id;
    });

    // Seed a running enhancer job with the taskId
    await t.run(async (ctx) => {
  ...
  test('no-ops when role is enhancer but no running job links to the pending task', async () => {
    const { sessionId } = await createTestSession('stta-enhancer-no-job');
    const chatroomId = await createChatroom(sessionId, ['enhancer']);
    await joinParticipant(sessionId, chatroomId, 'enhancer');
