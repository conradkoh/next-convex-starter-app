...
function looksLikeHtmlDecoded(value: string) { return !!value && !containsFencedCode(value) && LEGACY_HTML_TAG_PATTERN.test(value); }
...
export function normalizeMarkdownContent(input: string) { const value = input.trim(); if (!value) return ''; const decoded = decodeHtmlEntities(value); return looksLikeHtmlDecoded(decoded) ? htmlToMarkdown(decoded) : decoded; }
